...
class TrajectoryLayerFactory extends ALspSingleLayerFactory {
  ...
  public ILspLayer createLayer(ILcdModel aModel) {
    return TLspShapeLayerBuilder.newBuilder()
                                ...
                                .bodyStyler(TLspPaintState.REGULAR, TLspLineStyle.newBuilder()
                                                                                 ...
                                                                                 .opacity(0.03f)
                                                                                 .blendOverlap(TLspLineStyle.BlendOverlapMode.ALWAYS)
                                                                                 .elevationMode(ILspWorldElevationStyle.ElevationMode.ABOVE_ELLIPSOID)
                                                                                 .width(2.0f).build())
                                .selectable(false).build();
  }

  @Override
  public boolean canCreateLayers(ILcdModel aModel) {
    return aModel.getModelDescriptor().getDisplayName().equals("Trajectory lines");
  }
}

 ...
 */
public class InteractiveSwingLabelComponent extends JPanel {
  ...
  // Listeners
  private List<MouseListener> fCrossIconMouseListeners = new CopyOnWriteArrayList<MouseListener>();
  private List<MouseListener> fTickIconMouseListeners = new CopyOnWriteArrayList<MouseListener>();
  ...
  public void setObject(Object aObject, ILcdModel aModel) {
  ...
  public boolean commitCommentChanges() {
    ...
    String oldComment = fCitiesCommentMap.put(fCurrentObject, comment);
    boolean changed = !(comment == null ? "" : comment).equals(oldComment == null ? "" : oldComment);
    if (changed && fCurrentModel != null) {
      fCurrentModel.elementChanged(fCurrentObject, ILcdModel.FIRE_NOW);
    }
    return changed;
  }
  ...
  public boolean dispatchMouseEvent(MouseEvent aMouseEvent) {
    Component comp = aMouseEvent.getComponent();
    if (comp instanceof MyLabel) {
      comp.dispatchEvent(aMouseEvent);
      return true;
    } else if (comp == fCommentsField) {
      comp.dispatchEvent(aMouseEvent);
      return true;
    }
  ...
  public boolean dispatchAWTEvent(AWTEvent aEvent) {
    if (aEvent instanceof MouseEvent) {
      MouseEvent mouseEvent = (MouseEvent) aEvent;
      return dispatchMouseEvent(mouseEvent);
    } else {
      return false;
    }
  }

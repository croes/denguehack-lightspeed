...
public class GARSCoordinateLayer extends TLcdGXYLayer implements StatusedCoordinateLayer {

  private final GARSGridLayer fGridLayer;

  public GARSCoordinateLayer(GARSGridLayer aGridLayer) {
    ...
    MultilevelGridElementPainter area_painter = new MultilevelGridElementPainter(aGridLayer.getGARSGrid());
  ...
  public StatusedEditableMultilevelGridCoordinate createStatusedEditableMultilevelGridCoordinate() {
    return new StatusedGARSCoordinate();
  }

  private static ILcdModel createModel(GARSGridLayer aGridLayer) {
    GARSGrid grid = aGridLayer.getGARSGrid();
    ILcdGeodeticDatum datum = ((ILcdGeoReference) aGridLayer.getModel().getModelReference()).getGeodeticDatum();
    GARSCoordinateModel area_model = new GARSCoordinateModel(datum);
    area_model.setModelDescriptor(new TLcdModelDescriptor("", "GARS areas", "GARS areas"));
    return area_model;
  }

  @Override
  public GARSGridLayer getGridLayer() {
    return fGridLayer;
  }
}

...
/**
 * A map from long to long values, which is additionally indexed to be able to perform fast
 * look-ups in large maps.
 */
public class LongLongIndexedMap {

  private String fBaseDir;
  private String fSourceName;
  private ILcdInputStreamFactory fInputStreamFactory;

  private List<Long> fBaseIndices = new ArrayList<Long>();
  private List<SoftReference<SubMap>> fIndexList = new ArrayList<SoftReference<SubMap>>();

  public LongLongIndexedMap(String aSourceName,
    ...
    while (dis.available() > 0) {
      fBaseIndices.add(dis.readLong());
      fIndexList.add(null);
    }
  }

  public long getValue(long aKey) {
    int fileIndex = 0;
  ...
  private SubMap readSubMap(int aIndex) throws IOException {
    ...
    int nrValues = (int) file.length() / 16;
    long[] keys = new long[nrValues];
  ...
  private static class SubMap {
    ...
    private SubMap(long[] aKeys, long[] aValues) {
      fKeys = aKeys;
      fValues = aValues;
    }

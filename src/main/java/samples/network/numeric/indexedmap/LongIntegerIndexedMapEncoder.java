...
/**
 * An encoder for converting a single file containing a long-integer mapping into an indexed 
 * structure with multiple smaller files.
 */
public class LongIntegerIndexedMapEncoder {
  ...
  public void buildIndexedMap(String aSourceName,
    ...
    for (SortedMap.Entry<Long, Integer> entry : map.entrySet()) {
      if (counter % 10000 == 0) {
        if (dos1 != null) {
          dos1.close();
        }

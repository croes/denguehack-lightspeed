...
/**
 * An encoder for converting a single file containing an integer-long mapping into an indexed
 * structure with multiple smaller files.
 */
public class IntegerLongIndexedMapEncoder {
  ...
  public void buildIndexedMap(String aSourceName,
    ...
    for (SortedMap.Entry<Integer, Long> entry : map.entrySet()) {
      if (counter % 10000 == 0) {
        if (dos1 != null) {
          dos1.close();
        }
        ...
        dos2.writeInt(entry.getKey());
        fileCounter++;
      }
      dos1.writeInt(entry.getKey());

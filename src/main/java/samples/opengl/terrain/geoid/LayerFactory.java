...
class LayerFactory implements ILcdGLLayerFactory {
  ...
  static TLcdGLAboveGroundPointlistPainter fAboveGroundPainter = new TLcdGLAboveGroundPointlistPainter();
  ...
  public ILcdGLLayer createLayer( ILcdModel aModel, ILcdGLView aTargetView ) {
    ...
    if ( typeName.equals( ModelFactory.GRID_MODEL_TYPE_NAME ) ) {
      ILcdGLLayer grid_layer = GLViewSupport.createGridLayer( aModel, aTargetView );
      // Do not paint the grid layer filled.
      if ( grid_layer.getPainter() instanceof TLcdGLPaintablePainter ) {
        ( ( TLcdGLPaintablePainter ) grid_layer.getPainter() ).setPaintFill( false );
      }
      return grid_layer;
    }
    else {

...
import com.luciad.view.opengl.action.TLcdGLViewCenterAction;

/**
 * <p>Action which allows to center the view on multiple layers.</p>
 */
public class CenterLayerAction extends AbstractLayerTreeAction {
  private static String STRING_CENTER_LAYER = "Center view on layer";

  private ILcdGLView fGLView;
  private TLcdGLViewCenterAction fGLViewCenterAction = new TLcdGLViewCenterAction();

  /**
   * <p>Create an action to center the view on multiple layers.</p>
   *
   * @param aLayered the <code>ILcdTreeLayered</code> to create the action for
   * @param aGLView  the view which will be centered on the layer(s)
   */
  public CenterLayerAction( ILcdTreeLayered aLayered, ILcdGLView aGLView ) {
    super( aLayered );
    if ( aGLView == null ) {
      throw new NullPointerException( "GLView may not be null" );
    }
    fGLView = aGLView;
    setIcon( new TLcdGUIIcon( TLcdGUIIcon.CROSS16 ) );
    setShortDescription(STRING_CENTER_LAYER);
  }
  ...
  public void actionPerformed( ActionEvent e ) {
    if ( fGLView != null ) {
      ...
      if ( bounds != null ) {
        fGLViewCenterAction.setBounds( bounds );
        fGLViewCenterAction.setView( fGLView );
        fGLViewCenterAction.actionPerformed( null );
      }
    }
  }

...
/**
 * {@link ALcyFormatBarFactory} that creates {@link samples.lucy.symbology.common.SymbologyFormatBar} instances for a GXY view.
 */
public abstract class GXYSymbologyFormatBarFactory extends ALcyFormatBarFactory {
  ...
  protected GXYSymbologyFormatBarFactory(ALcyFormat aSymbologyFormat,
  ...
  private ILcyMapComponent findMapComponent(ILcdGXYView aView) {
    return (ILcyMapComponent) fLucyEnv.getCombinedMapManager().findMapComponent(aView);
  }

  @Override
  public boolean canCreateFormatBar(ILcdView aView, ILcdLayer aLayer) {
    return aView instanceof ILcdGXYView && findMapComponent((ILcdGXYView) aView) != null;
  }

  @Override
  public ALcyFormatBar createFormatBar(ILcdView aView, ILcdLayer aLayer) {
    ILcyMapComponent map = findMapComponent((ILcdGXYView) aView);
    return new GXYSymbologyFormatBar(fSymbologyFormat,
                                     map,
                                     fToolBarID,

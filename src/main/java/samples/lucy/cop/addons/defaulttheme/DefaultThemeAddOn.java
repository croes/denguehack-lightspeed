 ...
 */
package samples.lucy.cop.addons.defaulttheme;
...
/**
 * Add-on which provides the default theme to the theme manager
 */
public class DefaultThemeAddOn extends AThemeAddOn {

  public DefaultThemeAddOn() {
    super(ALcyTool.getLongPrefix(DefaultThemeAddOn.class),
          ALcyTool.getShortPrefix(DefaultThemeAddOn.class));
  }

  @Override
  protected Theme createTheme(final ILcyLucyEnv aLucyEnv) {
    Theme defaultTheme = new DefaultTheme();
    return new ClassifiedThemeDecorator(
        new ThemeWithConfiguredLayersDecorator(defaultTheme, aLucyEnv, getShortPrefix(), getPreferences(), new ILcdMorphingFunction<String, String>() {
          @Override

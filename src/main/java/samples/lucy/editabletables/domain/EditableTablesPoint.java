 ...
 */
package samples.lucy.editabletables.domain;
...
/**
 * Extension of point that also implements ILcdDataObject.
 */
public class EditableTablesPoint extends TLcdLonLatPoint implements ILcdDataObject {

  private final TLcdDataObject fDataObject;

  public EditableTablesPoint(double aX, double aY) {
    super(aX, aY);
    fDataObject = new TLcdDataObject(DATA_TYPE);

    // Initialize the nested data object, otherwise we can't update its values.
    setValue(DATA_OBJECT_PROPERTY, SUB_DATA_TYPE.newInstance());
  }

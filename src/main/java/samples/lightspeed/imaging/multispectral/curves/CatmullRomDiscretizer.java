...
/**
 * Discretizer for CatmullRomEditLine
 */
public class CatmullRomDiscretizer extends TLspShapeDiscretizer {
  @Override
  public void discretizeSFCT(Object aDomainObject, TLspShapeDiscretizationParameters aParameters, TLspContext aContext, ALspEditable3DMesh a3DMeshSFCT) throws TLspDiscretizationException {
    if (aDomainObject instanceof CatmullRomCurve) {
      ILcdBounds bounds = ((CatmullRomCurve) aDomainObject).getBounds();
      double maxEdgeLen = (bounds.getWidth() + bounds.getHeight()) * 0.5 * 0.1;
      if (aParameters.getMaximalEdgeLength() > maxEdgeLen) {
        aParameters = new TLspShapeDiscretizationParameters.Builder().
                                                                         all(aParameters).
                                                                         maximalEdgeLength(maxEdgeLen).
                                                                         build();
      }
    }

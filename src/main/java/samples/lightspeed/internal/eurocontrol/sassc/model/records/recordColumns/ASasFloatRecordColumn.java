...
/**
 * Abstract base class for single- and multi-valued float record columns.
 *
 * @author Bruno Christiaen
 */
public abstract class ASasFloatRecordColumn extends ASasRecordColumn {

  /**
   * Float value used to represent NULL in DB.
   */
  protected static final float NULL_VALUE = Float.MIN_VALUE;
  protected static final float DEFAULT_VALUE = 0;

  /**
   * Create a new record column based on the given definition.
   * @param aDefinition The record column definition.
   */
  public ASasFloatRecordColumn(TSasDbExpressionDefinition aDefinition) {
    super(aDefinition);
  }
  ...
  /**
   * Float value used to indicate a NULL value in the database.
   * @return The value.
   */
  public static float getNullValue() {
    return NULL_VALUE;
  }
   ...
   */
  protected static float parseDBValue(Object aValue) {
    float value;
    if (aValue == null) {
      value = NULL_VALUE;
    } else {
      value = ((Number) aValue).floatValue();
    }
    return value;
  }
}

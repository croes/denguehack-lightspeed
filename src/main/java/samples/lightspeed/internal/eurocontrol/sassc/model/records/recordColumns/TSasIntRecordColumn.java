...
/**
 * Record columns for wich all elements are int.
 * @author patrickd
 *
 */
public class TSasIntRecordColumn extends ASasIntRecordColumn {
   ...
   */
  public TSasIntRecordColumn(TSasDbExpressionDefinition aDefinition) {
    this(aDefinition, 0);
  }
   ...
   */
  public TSasIntRecordColumn(TSasDbExpressionDefinition aDefinition, int aSize) {
    super(aDefinition);
    fValues = new int[aSize];
  ...
  public int getValue(int aIndex) {
    ...
    if (isNull(aIndex)) {
      return getDefinition().hasDefaultValue() ? Integer.parseInt(getDefinition().getDefaultNullValue()) : DEFAULT_VALUE;
    }
  ...
  public boolean isNull(int aIndex) {
    return (fValues[aIndex] == NULL_VALUE);
  }
  ...
  protected void storeValues(ObjectOutputStream aOutputStream) throws IOException {
    ...
    for (int i = 0; i < fValues.length; i++) {
      aOutputStream.writeInt(fValues[i]);
    }
  }

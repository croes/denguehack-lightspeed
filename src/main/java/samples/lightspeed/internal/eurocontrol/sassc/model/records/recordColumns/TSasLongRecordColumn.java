...
/**
 * Record columns for wich all elements are long.
 *
 * @author patrickd
 */
public class TSasLongRecordColumn extends ASasLongRecordColumn {

  private long[] fValues;

  public TSasLongRecordColumn(TSasDbExpressionDefinition aDefinition, int aSize) {
    super(aDefinition);
    fValues = new long[aSize];
  ...
  public long getValue(int aIndex) {
    ...
    if (isNull(aIndex)) {
      return getDefinition().hasDefaultValue() ? Long.parseLong(getDefinition().getDefaultNullValue()) : DEFAULT_VALUE;
    }
  ...
  protected void storeValues(ObjectOutputStream aOutputStream) throws IOException {
    ...
    for (int i = 0; i < fValues.length; i++) {
      aOutputStream.writeLong(fValues[i]);
    }
  }
  ...
  protected void loadValues(ObjectInputStream aInputStream)
    ...
    fValues = new long[nrValues];
    for (int i = 0; i < nrValues; i++) {
      long value = aInputStream.readLong();
      fValues[i] = value;
    }
  }

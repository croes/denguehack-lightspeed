...
/**
 * Action to add TLspLayerTreeNodes to a layer tree
 */
public class AddLspLayerTreeNodesAction extends AbstractAddLayerTreeNodesAction {

  /**
   * <p>Create an action to add a <code>TLspLayerTreeNode</code> to an
   * <code>ILcdTreeLayered</code>
   *
   * @param aLayered the <code>ILcdTreeLayered</code> to create the action for
   */
  public AddLspLayerTreeNodesAction(ILcdTreeLayered aLayered) {
    super(aLayered);
  }

  @Override
  protected ILcdLayerTreeNode createNode(String aLabel) {
    return new TLspLayerTreeNode(aLabel);
  }

...
import samples.gxy.touch.touchEvents.DeviceEvent;
...
public class TouchEventFactory implements DeviceEventListener {
  ...
  /**
   * Create a new touch point based on the provided device event.
   *
  ...
  /**
   * Create a new touch event based on the provided touch point and timestamp.
   *
   * @param aTouchPoint the touch point.
  ...
  private TLcdTouchEvent createTouchEvent(TLcdTouchPoint aTouchPoint,
    ...
    case DOWN: {
      addTouchPoint(aTouchPoint);
      return createTouchEventImpl(aTouchPoint, aTimeStamp);
    }
    case MOVED: {
      replaceTouchPoint(aTouchPoint);
      return createTouchEventImpl(aTouchPoint, aTimeStamp);
    }
    case UP: {
      removeTouchPoint(aTouchPoint.getID());
      return createTouchEventImpl(aTouchPoint, aTimeStamp);
    }
    default: {
      // May never happen.
  ...
  /**
   * Creates a new touch event for the given touch point and time stamp.
   *
   * @param aTouchPoint the touch point.
   * @param aTimeStamp the timestamp.
   *
  ...
  /**
   * Adds the given touch point to the list of current touch points.
   * @param aTouchPoint a touch point.
   */
  private void addTouchPoint(TLcdTouchPoint aTouchPoint) {
    fCurrentlyKnowTouchPoints.add(aTouchPoint);
  }

  /**
   * Replace the touch point with the given ID in the list of current touch points.
   * @param aTouchPoint a touch point.
   */
  private void replaceTouchPoint(TLcdTouchPoint aTouchPoint) {
    removeTouchPoint(aTouchPoint.getID());
    addTouchPoint(aTouchPoint);
  }

  /**
   * Removes the given touch point from the list of current touch points.
   * @param aTouchPointID a touch point.
   */
  private void removeTouchPoint(long aTouchPointID) {
  ...
  /**
   * Checks if a touch point with the given ID exists in the list of current touch points.
   * @param aTouchPointID a touch point.
   * @return if a touch point with the given ID exists.
   */

...
/**
 * The default model factory uses the service registry.
 */
public class DefaultFormatModelFactory extends AbstractModelFactory {

  private ILcdInputStreamFactory fInputStreamFactory = new TLcdInputStreamFactory();
  private Iterable<ILcdModelDecoder> fModelDecoders = ServiceRegistry.getInstance().query(ILcdModelDecoder.class);

  public DefaultFormatModelFactory(String aType) {
    super(aType);
  }

  @Override
  public ILcdModel createModel(String aSource) throws IOException {
    for (ILcdModelDecoder decoder : fModelDecoders) {
      if (decoder.canDecodeSource(aSource)) {
        synchronized (decoder) {
          if (decoder instanceof ILcdInputStreamFactoryCapable) {
            ((ILcdInputStreamFactoryCapable) decoder).setInputStreamFactory(fInputStreamFactory);
          }

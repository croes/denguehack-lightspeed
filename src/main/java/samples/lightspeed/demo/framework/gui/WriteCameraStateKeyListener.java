 ...
 * pressed.
 */
class WriteCameraStateKeyListener implements KeyListener {

  private static ILcdLogger sLogger = TLcdLoggerFactory
      .getLogger(WriteCameraStateKeyListener.class);

  private ILspView fView;

  public WriteCameraStateKeyListener(ILspView aView) {
    fView = aView;
  }
  ...
  public void keyReleased(KeyEvent e) {
    if (e.getKeyChar() == 'N') {
      ...
      if (fView.getViewXYZWorldTransformation() instanceof TLspViewXYZWorldTransformation2D) {
        CameraFileUtil.write2DCamera(fView, ThemeAnimation.get2DCameraFile(framework.getActiveTheme()));
      } else {
        CameraFileUtil.write3DCamera(fView, ThemeAnimation.get3DCameraFile(framework.getActiveTheme()));
      }
    }
  }
}

...
/**
 * An action that displays a line-of-sight input panel in a dialog.
 */
class ShowLOSPanelAction extends ALcdAction {
  ...
  ShowLOSPanelAction( LOSPanel aLOSPanel, Frame aOwnerFrame, ALcdTerrainElevationProvider aTerrainElevationProvider ) {
    fLOSPanel = aLOSPanel;
    fOwnerFrame = aOwnerFrame;
    fComputeButton.addActionListener( new CreateLOSAction( fLOSPanel, aTerrainElevationProvider ) );
    ...
    setName( "Compute raster line-of-sight" );
    setShortDescription( "Compute raster line-of-sight" );
  }
  ...
  public void actionPerformed( ActionEvent e ) {
    ...
    if ( fDialog == null ) {
      ...
      fDialog = new JDialog( fOwnerFrame, "Line-of-sight settings" );
      fDialog.getContentPane().setLayout( new BorderLayout() );
      fDialog.getContentPane().add( fLOSPanel, BorderLayout.NORTH );

    ...
    public ILcdModel decode(String aSource) throws IOException {
        ...
        try {
            ...
            while (scanner.hasNext() && lineIndex < maxLine) {
                List<String> values = split(scanner.nextLine());
                List<String> strippedValues = new ArrayList<String>(values.size());
                for(int i = 0; i < values.size(); i++) {
                    strippedValues.add(values.get(i).replaceAll("\"", EMPTY_VALUE));
                }
